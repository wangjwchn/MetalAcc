...
class  ImageSobel: AccImageFilter {
    override init(){
        super.init()
        self.name = "MPSImageSobel"
    }
    override func applyFilter() {
        let commandBuffer = self.base!.commandQueue!.commandBuffer()
        let imagesobel = MPSImageSobel(device: self.base!.device!)
        imagesobel.encodeToCommandBuffer(commandBuffer, sourceTexture: self.base!.inTexture!, destinationTexture: self.base!.outTexture!)
        commandBuffer.commit()
        commandBuffer.waitUntilCompleted()
    }
